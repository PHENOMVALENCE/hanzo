...
if (!$category) {
    ...
    echo '<main class="container py-5"><p>Category not found.</p><a href="' . e(app_url('categories.php')) . '">Back to categories</a></main>';
    $footerMode = 'full';
...
$sql = 'SELECT p.*, c.name AS category_name FROM products p JOIN categories c ON c.id = p.category_id WHERE ' . implode(' AND ', $where) . ' ORDER BY ' . $orderSql;
$st = $pdo->prepare($sql);
$st->execute($params);
            ...
            <div class="hanzo-sidebar p-0 mb-3">
                ...
                <div class="list-group list-group-flush">
                    <?php foreach ($sidebarCats as $sc): ?>
                        <a class="list-group-item list-group-item-action <?= (int) $sc['id'] === $id ? 'active' : '' ?>" href="<?= e(app_url('category.php?id=' . (int) $sc['id'])) ?>"><?= e($sc['name']) ?></a>
                    <?php endforeach; ?>
                </div>
            </div>
        </aside>
        <div class="col-lg-9">
            <h1 class="h3 mb-2"><?= e($category['name']) ?></h1>
            ...
            <form class="row g-2 align-items-end bg-white border rounded p-3 mb-4" method="get" action="">
                ...
                <div class="col-md-2">
                    <button type="submit" class="btn btn-hanzo-primary w-100">Apply</button>
                </div>
            </form>
